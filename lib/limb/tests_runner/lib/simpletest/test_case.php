...
class SimpleTestCase {
    ...
    private $methods_include_filters = array();
    ...
    function filter(array $methods_filters) {
        $this->methods_include_filters = array_map('strtolower', $methods_filters);
    }
    ...
    protected function isTest($method) {
        if (strtolower(substr($method, 0, 4)) == 'test') {
            if(count($this->methods_include_filters)
                    && !in_array(strtolower($method), $this->methods_include_filters))
                return false;
